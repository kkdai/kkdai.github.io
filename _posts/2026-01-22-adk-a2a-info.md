---
layout: post
title: "[Google ADK][A2A] LINE Bot 架構大改造：從 if/elif 地獄到 Multi-Agent Orchestration"
description: "使用 Google Agent Development Kit (ADK) 重構 LINE Bot，實現 Agent-to-Agent (A2A) 通訊架構"
category:
- TIL
...
在維護 [linebot-helper-python](https://github.com/kkdai/linebot-helper-python) 專案時，我一直面臨一個架構問題：隨著功能增加，`main.py` 中的 `if/elif` 判斷式越來越長，程式碼越來越難維護。
...
**❌ 難以維護** - 新增功能就要加 elif，檔案越來越肥
**❌ 難以測試** - 所有邏輯混在一起，單元測試困難
**❌ 難以擴展** - 想要並行處理多個意圖？改起來很痛苦
**❌ 職責不清** - 對話、摘要、位置搜尋全部混在一起

在 2025 年初，Google 發布了 [Agent Development Kit (ADK)](https://google.github.io/adk-docs/)，提供了建構 Multi-Agent 系統的框架。我決定用 ADK 重構整個專案，實現 Agent-to-Agent (A2A) 架構。
...
## Phase 1: 安裝 ADK 與重構 Tools (Release v0.3.0)
...
這次遷移最重要的決策是**分 5 個階段進行**，每個階段都是獨立的 PR：
...
- [Google Agent Development Kit (ADK)](https://google.github.io/adk-docs/) - 官方文件
- [Vertex AI Gemini API](https://cloud.google.com/vertex-ai/docs/generative-ai/model-reference/gemini) - Gemini 模型文件
- [linebot-helper-python Repository](https://github.com/kkdai/linebot-helper-python) - 專案原始碼
