---
layout: post
title: "[AI 實戰] 用 Claude Code 打磨 macOS 會議翻譯 App：自動重連、懸浮字幕、會議記錄匯出全面進化"
description: "延續上一篇 AGY CLI 建構會議翻譯 App 的成果，這次改用 Anthropic 的 Claude Code，透過 Subagent 驅動開發，修復 10 分鐘斷線謎題、新增懸浮字幕視窗與會議記錄自動匯出，並用 Python 生成專業 App Icon，記錄一場更深度的人機協作體驗。"
category:
- AI
- DeveloperExperience
...
Claude Code 閱讀了全部五個 Swift 原始檔，並結合內建的 Gemini Live API 技能文件，精準指出問題根源：
...
Claude Code 隨即重構了 `GeminiLiveConnection.swift`，在不改變任何外部介面的前提下，引入三道防線：
...
> **Claude Code**: Build 成功，commit 完成，push 上去了。之後即時翻譯應該可以長時間持續運作，Session 到期時自動靜默重連，使用者不會察覺中斷。
...
Claude Code 仔細閱讀程式碼後，先提出一個定向問題，而非直接給出清單：
...
接下來是一場簡短的需求訪談，Claude Code 一次只問一個最關鍵的問題：

- 懸浮視窗要顯示幾行？→ **雙行（原文小字 + 翻譯大字）**
- 背景風格？→ **毛玻璃效果（vibrancy）**
- 匯出方式？→ **自動存到桌面，不跳對話框**

五個問題之後，設計方向完全清晰。Claude Code 提出完整的設計方案並撰寫了規格文件，存入版本庫後，開發者確認「沒問題」，進入實作階段。

---

# 階段三：計畫驅動開發 — Subagent 閉環交付，Review 抓出關鍵 Bug

有了明確規格，Claude Code 進入了它最擅長的工作模式：**先寫計畫，再用多個獨立 Subagent 分工執行，每個 Task 完成後立即由 Reviewer Subagent 審查**。
...
> **Claude Code**: Build 成功、commit 完成、push 上去了。
...
Claude Code 先確認環境中有 `Pillow`（Python 圖像函式庫），接著直接動手寫了一個完整的 Icon 生成腳本，設計說明如下：
...
Claude Code 仔細執行 Build 並分類了三種警告，依序對症下藥：
...
Claude Code 閱讀 `AudioCaptureManager.swift` 後，第一眼就發現問題：`fetchShareableApps()` 呼叫 `SCShareableContent.current` 失敗時，只會 `print` 到 console，UI 顯示空列表但毫無任何提示。開發者完全不知道發生了什麼事。
...
Claude Code 修正了判斷邏輯——只有 ScreenCaptureKit 確切的 `userDeclined` 錯誤碼（`-3801`）才視為權限問題，其他錯誤一律顯示真實的 domain、code 與描述，方便診斷：
...
修正 error 分類後，Claude Code 執行 App 並擷取 log，發現狀態列顯示的是帶有 code 編號的新訊息，不是 `-3801`。這確認了：**問題根本不是用戶沒給權限，而是 macOS 根本認不出這個 App**。
...
Ad-hoc 簽名的問題在開發期間會持續存在——每次 rebuild 都需要重新授權。Claude Code 把 `tccutil reset` 直接加進 `build_app.sh` 的最後一步：
...
這次與 Claude Code 的協作，讓我感受到與第一次 AGY CLI 開發截然不同的工作方式：
